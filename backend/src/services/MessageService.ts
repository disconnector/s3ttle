...
/**
 * Get session metadata (topic, creation date, participants).
...
/**
 * Create a session without saving a message.
 * Called when Partner A clicks "Start Decision" so the session exists
 * immediately, before any messages are sent.
 */
export function createSession(sessionId: string, topic: string): void {
  if (!SessionStore.getSession(sessionId)) {
    SessionStore.createSession(sessionId, topic);
  }
}

/**
 * Register a participant's display name and avatar in a session.
 * Called when a partner sends a message — records their identity so:
 * 1. The other partner's frontend can show their real name + avatar
 * 2. Claude can address them by name in responses
 *
 * @param sessionId - The session to update
 * @param role      - 'partner_a' or 'partner_b'
 * @param info      - Their display name, avatar emoji, and device ID
 */
export function registerParticipant(
  ...
  SessionStore.setParticipant(sessionId, role, info);
}
