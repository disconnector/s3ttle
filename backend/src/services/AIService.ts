 ...
 *
 * If we know the partners' real names (from their profile), we use those:
 *   "Rich: I think we should do it"
 *   "Sarah: I'm not so sure"
 *
 * If we don't have names yet, we fall back to "Partner A" / "Partner B".
 * The system prompt tells Claude to expect this format.
 *
 * @param messages  - The conversation history
 * @param nameMap   - Maps 'partner_a'/'partner_b' to display names (if known)
 */
function formatMessagesForClaude(
  ...
  return messages.map((msg) => {
    ...
    // Use real name if we have it, otherwise fall back to generic label
    const label = nameMap.get(msg.author)
      ?? (msg.author === 'partner_a' ? 'Partner A' : 'Partner B');
...
export async function getAIResponse(sessionId: string): Promise<AIResponse> {
  ...
  try {
    // ── Step 2: Load Conversation History + Participant Names ──────────────
    const history = MessageService.getConversationHistory(sessionId);
    const session = MessageService.getSession(sessionId);

    // Build a name map from the session's registered participants.
    // This lets us label messages with real names (e.g. "Rich:" instead of "Partner A:").
    const nameMap = new Map<string, string>();
    if (session?.participants) {
      for (const [role, info] of session.participants) {
        nameMap.set(role, info.displayName);
      }
    }

    const claudeMessages = formatMessagesForClaude(history, nameMap);

    // ── Step 3: Enhance System Prompt with Partner Names ────────────────
    // If we know the partners' names, append them to the system prompt so
    // Claude addresses people by name. This makes the facilitation feel
    // personal rather than generic "Partner A" / "Partner B".
    let systemPrompt = FACILITATOR_SYSTEM_PROMPT;
    const nameA = nameMap.get('partner_a');
    const nameB = nameMap.get('partner_b');
    if (nameA || nameB) {
      systemPrompt += `\n\n## Partner Names\n`;
      if (nameA) systemPrompt += `- Partner A's name is ${nameA}. Address them as ${nameA}.\n`;
      if (nameB) systemPrompt += `- Partner B's name is ${nameB}. Address them as ${nameB}.\n`;
      systemPrompt += `Messages are labeled with these names (e.g. "${nameA || 'Partner A'}: ..."). Use their real names in your responses.`;
    }
    ...
    const response = await getClient().messages.create({
      ...
      system: systemPrompt,               // Instructions + partner names
