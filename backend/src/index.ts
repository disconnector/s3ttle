...
// Bind to '0.0.0.0' so the server accepts connections from ANY network interface,
// not just localhost. This is essential for Partner B to connect from another device
// on the same Wi-Fi network. Without this, only the machine running the server
// could access the API.
app.listen(Number(PORT), '0.0.0.0', () => {
  console.log(`[S3ttle] Server running on 0.0.0.0:${PORT}`);
});
