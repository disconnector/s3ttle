 ...
 *
 * This is shown AFTER the one-time profile setup. It has two flows:
 *
 ...
 *
 * The user's profile (name + avatar) is shown as a greeting to confirm identity.
...
import { clearProfile, type UserProfile } from '@/lib/profile';

interface SetupViewProps {
  onStart: (topic: string, role: 'partner_a' | 'partner_b', sessionId: string) => void;
  profile: UserProfile;  // The user's saved profile (name + avatar)
  onSwitchProfile: () => void;  // Called when user wants to switch identity
}

export function SetupView({ onStart, profile, onSwitchProfile }: SetupViewProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-dvh bg-background px-4">
      {/* Settings button pinned to top-right corner */}
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          >
            Hey {profile.avatar} {profile.displayName}! Ready to decide something?
          </motion.p>
          <button
            onClick={() => { clearProfile(); onSwitchProfile(); }}
            className="text-xs text-muted-foreground/60 hover:text-muted-foreground underline mt-1"
          >
            Not {profile.displayName}? Switch
          </button>
        </div>
